...
public class RPCServer extends NettyServer {
	...
	private void initProvider(int port) {
		...
		try {
			serviceProvider = (ServiceProvider) Class.forName(RegistryConfig.getInstance().getProvider())
	...
	private static class RPCInvokeHandler extends SimpleChannelInboundHandler<Message> {
		...
		private void invokeAndReturn(final ChannelHandlerContext ctx, final Message message, final RPCRequest requset,
				final RPCResult response) {
				...
				public void run() {
					try {
						...
						if (StringUtils.isBlank(requset.getImplementName())) {
							...
							result = invoker.invoke(requset.getClassName(), requset.getImplementName(),
									requset.getMethodName(), requset.getParamters(), requset.getParamterTypes());
						}

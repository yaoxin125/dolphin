...
/**
 * RPC服务的代理类，通过该代理，去调用远程的服务。 会根据RPCService注解上的value去服务中心取到该服务所在的地址，并进行通信。
 *
 * @author jiujie
...
public class RPCServiceProxy implements InvocationHandler {
	 ...
	 *
	 * @param implementName
	 *            接口实现类的名字，当一个远程接口有多个实现时需要有此参数
	 * @author jiujie 2016年7月12日 上午10:46:08
	 */
	 ...
	 * @param annotation
	 * @return
	 * @author jiujie 2016年7月18日 下午4:13:14
	 */
	 ...
	 * @author jiujie 2016年7月7日 下午4:47:00
	 */
	private RPCResult request(String group, String serviceName, String className, Method method, Object[] args)
			throws ServiceNotFoundException {
		...
		for (int i = 0; i < RETRY_TIMES; i++) {
			try {
				...
				if (connection == null) {
					logger.error("Service [group:" + group + "," + "name:" + serviceName + "]" + " not found.");
	 ...
	 * @return
	 * @throws RPCException
	 * @author jiujie 2016年7月18日 上午11:26:06
	 */
	private RPCResult sendRequest(Connection connection, RPCRequest request) throws RPCException {
		RPCResult result;
		result = (RPCResult) REQUSET_MANAGER.sysnRequest(connection, new Header(PacketType.RPC), request);
		return result;
	}
	...
	private void logAfter(String className, Method method, Object[] args) {
		if (logger.isInfoEnabled()) {
			...
			logger.info("Service [className:" + className + ", method:" + method.getName() + ", args:" + arguments
					+ "] invoke success.");
		}
	}
